class UISoldierHeader_PZ_Amalgamation extends UISoldierHeader config(UIModificationWOTC);

var config array<name> PsionicClasses;
...
function bool UnitIsPsionic(XComGameState_Unit Unit)
{
	local name chkName;
	local name TemplateName;

	TemplateName = Unit.GetSoldierClassTemplateName();

	if (TemplateName == '')
		return false;

	foreach default.PsionicClasses(chkName) {
...
public function PopulateData(optional XComGameState_Unit Unit, optional StateObjectReference NewItem, optional StateObjectReference ReplacedItem, optional XComGameState NewCheckGameState)
{
	...
	if (DisplayXP_Check != true) {
		...
		if (AddXPDisplay) {
			...
			if (class'OrderControl'.static.CheckForActiveMod("XPDisplay") == true) {
				DisplayXP = true;
			}
		}
	}
	...
	*/
	...
	if(UnitIsPsionic(Unit) || Unit.bHasPsiGift || PsiStat > 0) {
		shouldShowPsi = true;
	}
